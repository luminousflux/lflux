(function($) { 
    ...
    $.fn.fullTextArea = function(options) {
        this.each(function() {
            ...
            $(this).after(holder);
            $(holder).append(this);
            var button = $('<button style="position: absolute; right: 10px; bottom: 10px; background: black; background: rgba(0,0,0,.5); color: white; color: rgba(255,255,255,.8); border: none; font-size: 3em; border-radius: 10px;">⎙</button>');
            ...
            $(button).click(function() {
                ...
                var keypressfn = function() { $(textarea).val($(newta).val()); };
                $(newta).keypress(keypressfn);
                $(overlay).find('button').click(function() {
                    $(overlay).remove();
                    $(newta).unbind('keypress', keypressfn);
                });
